...
class TestPlotEmotionDistribution(unittest.TestCase):
    ...
    This class includes tests to ensure that the function handles various
    inputs correctly and produces the expected visual output
    indications via matplotlib's plotting functions.
    """
    ...
    def test_empty_input(self, mock_show):
        ...
        with self.assertLogs(level="WARNING") as log:
            ...
            self.assertIn(
                "No emotions to plot, the input list is empty.", log.output[0]
            )
            # Ensure that matplotlib's show function was not called,
            # as there's nothing to plot
    ...
    def test_single_emotion(self, mock_show):
        ...
        # indicating that a plot was made
        self.assertTrue(mock_show.called)

    @patch("matplotlib.pyplot.show")
    def test_multiple_emotions(self, mock_show):
        """
        Test that 'plot_emotion_distribution'
        correctly plots multiple different emotions.
