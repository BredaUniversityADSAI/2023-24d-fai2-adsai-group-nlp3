...
def episode_preprocessing(input_path,
                          sample_rate, 
...
def predict(
    ...
    Author - Wojciech Stachowiak
    """
    #model, label_decoder = mp.get_model(model_path)

    tokens, masks = preprocessing.preprocess_prediction_data(data, max_length = 30)
    emotions, probabilities = me.predict(model, tokens, masks, label_decoder)

    #emotions, probabilities = mp.predict(model, tokens, masks, label_decoder)
...
def train_endpoint(val_size: float, epochs: int, 
                   lr: float, early_stopping_patience: int,
                   model_name:str, 
    ...
    model, label_decoder = model_training(train_data=input_path,
                                           val_size=val_size,
                                           epochs=epochs,
                                           lr=lr,
                                           early_stopping_patience=early_stopping_patience,
...
def evaluate_endpoint(model_name:str, test_data: UploadFile = File(...)):
    ...
    #        return {"error": "Model and label_decoder must be trained before evaluation."}
    ...
    evaluation_result = evaluate_model(test_data=input_path, model=model, label_decoder=label_decoder)
...
def predict_emotions(
    ...
    data_df = episode_preprocessing(input_path=input_path,
    ...
    emotions, probabilities = me.predict(model, tokens, masks, label_decoder)
    # Make predictions
    #predicted_emotions, highest_probabilities = predict(model=model,label_decoder=label_decoder, data=data_df)
...
#/{target_sr}/{segment_lenght}/{min_fragment_len}/{vad_aggressiveness}/{use_fp16}/{transcript_model_size}