...
def connect_to_azure_ml(subscription_id: str, resource_group: str, workspace_name: str):
    """
    Connects to the Azure Machine Learning workspace using
...
mt_logger = logging.getLogger("splitting")
...
def load_data(file_path: str) -> tuple[pd.DataFrame, dict[int, str]]:
    ...
    - emotion_decoder (dict[int, str]): A dictionary mapping each
    unique emotion to a numerical label.
    """
...
def get_train_val_data(
    ...
    Returns:
    - train_set (tuple[pd.DataFrame]): The training set containing
    the features and labels.
    - val_set (tuple[pd.DataFrame]): The validation set containing
    the features and labels.
    """
...
def main(
    ...
    if local:
        ...
        workspace, ml_client = connect_to_azure_ml(
            subscription_id, resource_group, workspace_name
        )
        _ = ml_client.datastores.get("workspaceblobstore")
